...
parser.add_argument("-S", "--signalHDU", default='DATA', type=str,
                    help=""" HDU name or number (0-indexed) containing the flux in fitscube
                    (default: 'DATA').  """)
parser.add_argument("-V","--varHDU", default='STAT', type=str,
                    help=""" HDU name or number (0-indexed) containing the variances in
...
parser.add_argument("-W","--width",default=151,type=int,
                    help=""" The median filter width in spectral pixels.  If an even number is
                    given, the actual filter value will be this number
                    +1. (Default: 151) """)
parser.add_argument("-G","--gwidth",default=0,type=float,
                    help=""" The sigma of a Gaussian kernel for in spectral pixels.  If not 0,
...
parser.add_argument("--passes", default=1, type=int,
                    help="""Number of filtering iterations.  Subsequent iterations
...
parser.add_argument("--ao",action='store_true',
                    help=""" Fill notch-filter blocked region with linear interpolated values.
...
parser.add_argument("--notch_start", type=float, default=5803.,
                    help=""" Start of the notch-filter blocked wavelength range (in Angstrom,
                    default: 5803).  """)
parser.add_argument("--notch_end", type=float, default=5970.,
                    help=""" End of the notch-filter blocked wavelength range (in Angstrom,
                    default: 5970).  """)
parser.add_argument("--notch_bound", type=int, default=30, 
                    help=""" Size of window before- and after notch-filter blocked region to
...
parser.add_argument("-t","--num_cpu",default=None,type=int,
                    help=""" Number of CPU cores to be used (default: all available).  """)
parser.add_argument("--savefiltered", action='store_true',
                    help=""" Also save the filtered data prior to subtraction.  Useful for
                    testing purposes.  """
)
parser.add_argument("--memmap",action='store_true',
                    help=""" Use memory mapping.  Reduces memory usage, but might also reduce
                    speed of execution.  """)
...
signalHDU = lef.int_or_str(args.signalHDU)  # hdu indices are passed to the programm in 0-indexed
...
if args.ao:
    ...
    while num_pass > 0:
        pass_count += 1 
        print(fitscube + ': Median filter - pass ' + str(pass_count)+ '/' +str(num_pass_total))
        num_pass -= 1
...
if args.ao:
    primary_header['HIERARCH LSD MFRAO'] = (1, 'notch-gap interpolated')
    primary_header['HIERARCH LSD AO_S'] = (args.notch_start, '[Angstrom] notch gap start')
    primary_header['HIERARCH LSD AO_E'] = (args.notch_end, '[Angstrom] notch gap end')
    primary_header['HIERARCH LSD AO_W'] = (args.notch_bound, ' width of notch interp. (pixel)')
...
med_sub_data_hdu = fits.ImageHDU(data=med_sub_data.astype(np.float32), header=med_sub_header)

if varHDU != '-1':
    variance, varhead = lef.read_hdu(fitscube, varHDU, memmap=memmap)
    variance_hdu = fits.ImageHDU(data=variance.astype(np.float32), header=varhead)
...
print(str(fitscube)+
      ...
      ' HDU 1: Original minus Medianf., (HDU2: Variances, HDU3: Medianf. if desired) - ' + \
      get_timestring(starttime))
