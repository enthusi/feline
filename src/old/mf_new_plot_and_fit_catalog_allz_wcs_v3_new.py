...
galaxy_absorption=[2586.65,2600.17,2796.35,3890.1506,3934.777,3969.588,4102.89,4305.61,4341.68,4862.68,5176.7,5895.6]
"""
...
def b0pressed(event):
  ...
  print "ok",px,py,z
  if foundz>0:
    verified_sources.write("%d \t%.1f \t%.1f \t%f  1 \t%.1f \t%d \t%d \t%d\t%f\t%f\t%f\t%d\t%f\t%f\t%f\t%.1f\t%.1f\n" % (run_id,px,py,z,verr,used,gtemplate,found,ra,dec,verr,quality,z,foundz,foundm,get_impact(qso_x,qso_y,px,py,z),foundb))
...
for line in catalog:
 ...
 # raw_wave=np.arange(raw_flux.wave.crval, raw_flux.wave.crval+raw_flux.wave.cdelt*raw_flux.wave.shape,raw_flux.wave.cdelt)
  raw_wave=np.arange(raw_flux.wave.get_crval(), raw_flux.wave.get_crval()+raw_flux.wave.get_step()*raw_flux.wave.shape,raw_flux.wave.get_step())
  print "fitting now"
  ...
  plt.title("(verr=%.1f), %d used lines, match strength=%d, b=%.1f" % (verr,used,quality,get_impact(qso_x,qso_y,px,py,z)))
  ...
  waven_high=np.arange(spec.wave.get_crval(), spec.wave.get_crval()+spec.wave.get_step()*spec.wave.shape,spec.wave.get_step()/10.0)
  print "xxx"
 ...
 # print "xxx"
  ...
  if False:
   ...
   try:
    ...
    plt.xlabel('radius')
    plt.ylabel('ERR')
    plt.tick_params(axis='both', left='off', top='off', right='off', bottom='on', labelleft='off', labeltop='off', labelright='off', labelbottom='on')
  ...
  wcs1=fullwhiteimage.wcs
  narrowsa = Image(data=all_ima.data, wcs=wcs1)[int(py)-aw/2:int(py)+aw/2,int(px)-aw/2:int(px)+aw/2]
  spic = plt.subplot2grid((rows,columns), (3,5))
  smoothnarrows=narrowsa.fftconvolve_gauss(center=None, flux=1.0, fwhm=(0.7, 0.7), peak=False, rot=0.0, factor=1, unit_fwhm=None, inplace=False)
  ...
  wcs1=fullwhiteimage.wcs
  narrows = Image(data=all_ima.data, wcs=wcs1)[int(py)-aw/2:int(py)+aw/2,int(px)-aw/2:int(px)+aw/2]
...
#whiteimage
  ...
  bestgauss=full_plane.gauss_fit(circular=True,pos_min=[aw-4,aw-4],pos_max=[aw+4,aw+10], unit_center=None, unit_fwhm=None)
