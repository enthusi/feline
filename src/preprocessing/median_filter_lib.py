...
def notch_interpol(spec, notch_start_px, notch_end_px, notch_bound):
    """
    mod_spec = notch_interpol(spec,
     filt_width, notch_start_px, notch_end_px, notch_bound)

    In:
    ---
    spec ... a spectrum, typically a datacube spaxel,
     as a 1D array
    ...
    notch_bound ... size of the window before notch_start_px
     and after notch_end_point
                    to determine the base-points via  median
                     over this spectral window.

    Out:
    ----
    mod_spec ... modified spectrum,
     filled with interpolated values over the notch-filter
                 blocked spectral bins.
    """

    # base points
    blue_base = np.median(
        spec[notch_start_px - notch_bound:notch_start_px + 1])
    red_base = np.median(
        spec[notch_end_px:notch_end_px + notch_bound + 1])
    ...
    int_inset = blue_base + np.indices(
        (notch_end_px - notch_start_px,))[0] * inc
...
def medfilt_cube_para(data, width, gwidth=0.,
    ...
    if verbose:
        print(str(fitscube) +
              ': Prepare the flux data for spectral'
              ' median and Gauss filtering... ' +
    ...
    if verbose:
        gwidth_info = (' / Post-Gauss filter g = ' +
                       str(gwidth)) if gwidth != 0 else ''
        print(str(fitscube) +
              ': Spectral filtering with a filter of W=' +
              str(width) + gwidth_info + '... ' +
              get_timestring(starttime))
    ...
    for j in range(num_threads):
        ...
        if j + 1 == num_threads:
            ...
            endstr = str(num_spec)
        print(str(fitscube) + ': Core #' + str(j + 1) +
              ': Median Filtering spectra ' +
              str(start + 1) + '-' + endstr)
    ...
    print(str(fitscube) +
          ': Done with the median filtering' + gf_str + ' of the' +
