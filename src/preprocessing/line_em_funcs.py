...
def write_primary(data, fheader, outfile):
    """
    Write out primary HDU with <data> and <fheader>
    into a FITS file named <outfile>:
    ...
    for line in ifile:
        ...
        if flag < 3:
            ...
            if star < 0.5:
                ...
                ofile.write('# text = {%(catno).5s} color = '
                            'yellow \n' % vars())
        else:
            ofile.write('ellipse(%(x).3f,'
                        ' %(y).3f, %(a).3f,'
...
def expand_cat(SX_input):
    ...
    fout.write('#   0 FITFLAG         If 1 the program'
               ' will fit this object\n')
    for line in fin:
        if line[0] == '#':
            fout.write(line)
        else:
...
def extract_spectra_circ(cube, x, y, r, extr_func=np.mean):
    ...
    extr_func controls which method is being used
     to calculate the output spectrum.
...
def ccs_header(ccs_routine, version, input_filename,
    ...
    cube_header['HIERARCH LSD CCS'] =\
        (ccs_routine, 'spatial cross-correlation (CCS) routine')
    cube_header['HIERARCH LSD CCSV'] = (version,
                                        'CCS version')

    cube_header = hierarch_multi_line(cube_header,
                                      'HIERARCH LSD CCSIN',
                                      input_filename,
                                      'CCS input filename')
    ...
    cube_header['HIERARCH LSD CCSPXSC'] = \
        (pix_scale, 'assumed pix scale arcsec/pix in datacube')
    cube_header = hierarch_multi_line(cube_header,
                                      'HIERARCH LSD CCMSK',
                                      mask_filename,
                                      'SSC mask filename')
    ...
    cube_header['HIERARCH LSD CCSFILT'] = \
        (filter_name, 'CCS filter funtion - i.e. Moffat or Gaussian')
    cube_header['HIERARCH LSD CCSFILTT'] = \
        (trunc_constant, 'CCS filter truncated at CCSFILTT x FWHM')
...
def mf_header(mfr_routine, version, width, gwidth, fitscube,
    ...
    header['HIERARCH LSD MFR'] = (mfr_routine,
