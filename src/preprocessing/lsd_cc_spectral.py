...
parser.add_argument("-i", "--input",
                    ...
                    containing the spatially
                    cross-correlated datacube
                    (output from lsd_cc_spatial.py).  """)
...
parser.add_argument("-e", "--expmap", type=str, default=None,
                    help="""
                    Name of the FITS file that contains the exposure
                     map N_exp(x,y)
                    (default: none).  If specified, the matched-filter output
                    SN(x,y,z) will be weighted SN'(x,y,z) =
                    sqrt(N_exp(x,y)/max(N_exp)) · SN(x,y,z),
                     where max(N_exp) will be
                    derived from the exposure map.  However,
                     you can also fix this
                    value with the --nmax argument
                    (usefull if it is not suitable to
                    extract the variance spectrum in the region
                     of maximum depth).
                    Note that --exmpap is ignored if
                     --classic mode is used.  """)
parser.add_argument("-F", "--FWHM",
                    type=float,
                    default=300,
                    help="""Specify the FWHM of the Gaussian line
...
parser.add_argument("-o", "--output",
                    ...
                    help=""" Name of the output FITS file.
                     The output FITS file will contain 2
...
parser.add_argument("-S",
                    ...
                    type=int_or_str,
                    default='DATA_2DCC',
                    help=""" HDU name or number (0-indexed) in
                     the input FITS file containing
                    the flux data. [Default: DATA_2DCC, i.e. the
                     output HDU from lsd_cc_spatial.py] """)
parser.add_argument("-N",
                    "--NHDU",
                    type=int_or_str,
                    default=None,
                    help=""" In normal mode: HDU name or number in
                     the variance spectrum FITS
...
parser.add_argument("-t",
                    ...
                    help=""" Number of CPU cores used in parallel operation.
                     [Default: all
                    available cpu cores] """)
parser.add_argument("--ignorenoise",
                    action="store_true",
                    help=""" Classic mode only: Switch to not
                     propagate the variance.  If set
                    the output FITS file will contain only 1 HDU
                     that stores the filtered
                    signal. (Note: In normal mode also one HDU
                     is written to disk, but
                    this is correctly normalised to be interpreted
                     as matched filter
                    output.) """)
parser.add_argument("--cunit3",
                    default='',
                    type=str,
                    help=""" Specify wavelength unit
                     ('Angstrom' or 'nm' or 'um'). [Default:
                    Value from FITS Header.]  """)
parser.add_argument("--nanfile",
                    type=str,
                    default='none',
                    help=""" Name of an FITS file that contains a 2D
                     image in` --nanmaskhdu`
                    (see below), that is of the same spatial dimensions
                     as the input cube.
                    Spectra corresponding to NaNs in this image will
                     be ignored in the
                    filtering. [Default: None] """)
parser.add_argument("--nanhdu", type=int_or_str,
                    default='4',
                    help=""" Number of HDU (0-indexed) or name
                     of FITS file specified in
                    --namask, where the 2D image is stored. [Default: 4] """)
parser.add_argument("--classic", action='store_true',
                    help="""LSDCat 1.0 mode: Cross-correlate data
                     with filter and propagate the
                    variance; cross-correlated data and propagated
                    varaince will be written to disk.
                     This requires a variance cube. """)
parser.add_argument("--notemp", action='store_true',
                    help="""In --classic mode, do not write out temporary
                     file to free memory
                    prior to propagating the variances.  This can speed up
                     things if you have
                    a lot of memory.
""")
parser.add_argument("--nmax", type=float, default=0,
                    ...
                    sqrt(N_exp(x,y)/nmax)) · SN(x,y,z),
                     where N_exp(x,y) are the
                    values of the exposure map provided with
                     the --expmap parameter.
                    If not set we use
                     SN'(x,y,z) = sqrt(N_exp(x,y)/max(N_exp)) ·
...
data_hdu = args.SHDU
if classic and args.NHDU is None:
    noise_hdu = 'STAT_2DCC'
elif not classic and args.NHDU is None:
    noise_hdu = 'VARSPEC'
else:
    noise_hdu = args.NHDU
...
if classic:
    print(program_name + ': LSDCat classic mode!'
                         '  Cross-correlatted data and propagated ' +
...
if not classic and args.varspec is not None:
    ...
    if len(var_spec) != data.shape[0]:
        print('ERROR: Mismatch of spectral dimensions between'
              ' variance spectrum (' +
              str(len(var_spec)) + ') and input cube (' +
              str(data.shape[0]) + ').')
        sys.exit(2)
elif not classic and not args.ignorenoise and args.varspec is None:
    print('ERROR: No variance spectrum FITS file provided (-v),'
          ' but --classic switch not set. ')
    sys.exit(2)
elif classic and args.varspec is not None:
    print(inputfile + ': --classic switch -- ignoring supplied'
                      ' 1D variance FITS file ' +
          args.varspec + ' / 3D variance will be read from HDU ' +
          str(noise_hdu) + ' of ' +
          inputfile + ' ...')
...
if args.nanfile != 'none':
    ...
    print(inputfile + ': No --nanfile set - all '
                      'spaxels will be filtered... ' +
          get_timestring(starttime))
    nans_select = None
...
if args.expmap is not None and not classic:
    ...
    primary_header = lef.copy_hist_header(primary_header,
                                          expmap_primary_header)
...
if not classic or args.ignorenoise:
    ...
    try:
        out_hdu_list.writeto(outfilename, overwrite=args.overwrite,
                             output_verify='ignore')
    except OSError:
        print('ERROR: Could not write output %s. '
              'Use --overwrite or different filename.' % outfilename)
        sys.exit(2)

    print(inputfile +
          ': All done! Filtered datacube stored in : ' +
          str(outfilename) + ' ' +
          get_timestring(starttime))
    ...
    print(inputfile +
          ': Preparing for writing out of the final datacube.... ' +
          get_timestring(starttime))
    ...
    print(inputfile +
          ': All done! Wavelength smoothed cube'
          ' & propagated noise stored in: ' +
