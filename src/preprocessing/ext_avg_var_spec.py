...
parser.add_argument("-o", "--output", type=str,
                    help=""" Name of the output FITS file containing the variance spectrum. 
                    [Default: eavs_+INPUT, i.e. eavs_ will be appended to the input file name]""")
parser.add_argument("-N", "--NHDU", type=int_or_str,
                    help="""HDU number (0-indexed) or name in the input FITS file containing
                    the variance data. [Default: 'STAT']""", default='STAT')
parser.add_argument("-x", "--xcen", type=float,
                    help="""X cooordinate (1-indexed) of the central spaxel of the extraction aperture.
                    [Default: Central spaxel along the x-axis of the
                    datacube]""", default=None)
parser.add_argument("-y", "--ycen", type=int,
                    help="""Y cooordinate (1-indexed) of the central spaxel of the extraction aperture.
                    [Default: Central spaxel along the y-axis of the
                    datacube]""", default=None)
parser.add_argument("-r", "--radius", type=float,
                    help=""" Radius of the circular extraction aperture (in spaxel).  [Default:
                    10] """, default=10.)
...
print(program_name+' Extracting an average variance spectrum from ' + args.input + \
      ' (HDU = '+str(args.NHDU)+')')

primary_header = fits.getheader(args.input, 0)
primary_header['HISTORY'] = "The average variance spectrum is from LSDCat2."
primary_header['HISTORY'] = "ext_avg_var_spec.py -- "+now.strftime("%m/%d/%Y, %H:%M:%S")
...
print(program_name+': Extracting at (x_cen,y_cen) = ('+str(xcen)+','+str(ycen)+')'+\
