...
import lib.spatial_smooth_lib as spatial_smooth_lib
# my own library with convenience functions
import lib.line_em_funcs as lef
...
if __name__ == "__main__":
    ...
    # workaround to allow for negative numbers not being treated
    ...
    wavelength dependent FSF kernel
     (approximated by a Moffat or a Gaussian).""",
                                     epilog="""
    Note:
    ...
    parser.add_argument("-i", "--input",
                        ...
                        type=str,
                        help="""
                        Name of the input FITS file containing the flux
                        (and variance) datacube.
                        """)
    ...
    parser.add_argument("-S", "--SHDU",
                        ...
                        default='1',
                        help="""
                        HDU name or number (0-indexed) in the input FITS file
                         containing the
                        flux data. [Default: 1]
                        """)
    parser.add_argument("-N", "--NHDU",
                        ...
                        default='2',
                        help="""
                        HDU number (0-indexed) or name in the input FITS file
                         containing the
    ...
    parser.add_argument("--std",
                        action='store_true',
                        help="""
                        Some noise cubes are std not variance (e.g. in KMOS).
                          If set the
    ...
    parser.add_argument("--ignorenoise",
                        action='store_true',
                        help="""
                        Switch to not propagate the variance.
                          If set the output FITS file
                        will contain only
                         1 HDU that stores the filtered signal.
                        """)
    ...
    parser.add_argument("-M", "--MHDU",
                        ...
                        default='0',
                        help="""
                        HDU number (0-indexed) or name of the mask
                        within MASK-file. The mask is supposed to be a 2D array
                         with the same spatial dimensions containing
                         only ones and zeros.  Spaxels corresponding to
                         zero-valued
                        pixels in the mask will be set to zero prior and
                        post the spatial
    ...
    parser.add_argument("-bc", default=['3.5'], nargs='*',
                        help="""
                        List of polynomial coefficients (whitespace seperated)
                        for the beta parameter of
                        the Moffat profile, i.e. -bc b0 b1 b2 ..
                        bn and for m>n bm == 0.
                        (default: 3.5).
                        """)
    parser.add_argument('-pc', default=['0.8'], nargs='*',
                        ...
                        (whitespace seperated) for the PSF FWHM,
    ...
    parser.add_argument("--lambda0",
                        ...
                        default=7050,
                        help="""
                        Zero-point of the polynomials, in Angstrom.
                          [Default: 7050 Angstrom]
                        """)
    parser.add_argument("--gaussian",
                        action='store_true',
                        help="""
                        Switch to use a Gaussian profile instead
                        of the default Moffat
                        profile as spatial filter profile.
                         The -bc parameter(s) will be
    ...
    parser.add_argument("-T", "--truncconstant",
                        ...
                        Parameter controlling the truncation of the filter
                         window: the filter
                        is truncated at T*WIDTH-PARAM - were WIDTH-PARAM =
                         sigma for Gaussian
    ...
    parser.add_argument("--notemp", action='store_true',
                        help="""In --classic mode, do not write out
                        temporary file to free memory
                        prior to propagating the variances.
                          This can speed up things if you have
                        a lot of memory.
    """)
    ...
    if selMask is False:
        ...
        print(program_name + ': Using mask cube in '
              + mask_filename + ' (HDU: ' +
              str(mask_hdu) + ')')
    print(program_name + ': PSF shape model = ' + filter_name)
    print(program_name + ': PSF FHWM(lambda) dependence via polynomial'
                         ' with coefficients ' +
          ', '.join(['p' + str(i[0]) + '=' + str(i[1]) for
                     i in enumerate(pc[::-1])]))
    if filter_name == 'Moffat':
        print(program_name + ': Moffat beta(lambda) dependence via polynomial'
                             ' with coefficients ' +
              ', '.join(['b' + str(i[0]) + '=' + str(i[1]) for
                         i in enumerate(bc[::-1])]))
    print(program_name + ': Reference wavelength lambda0 in '
                         'PSF polynomial(s): ' + str(lambda_0) + ' Angstrom')
    ...
    print(input_filename + ': Reading in the Data Cube... (HDU ' +
          str(data_hdu) + ') ' +
    ...
    cube_data, cube_header = lef.read_hdu(input_filename,
                                          ...
                                          nans_to_value=False,
                                          memmap=memmap)
    # first  we keep the nans, but
    # we set all NaNs to zero here,
    # otherwise the FFT filtering produces rubbish
    ...
    cube_data[nancube] = 0
    # note: with newer astropy > 3, convolve(...,nan_treatment='fill')
    ...
    try:
        ...
        cunit3 = 'Angstrom'
        print('WARNING: No CUNIT3 Keyword specifying the wavelength'
              ' unit found. Assuming Angstrom.')

    if selMask is True:
        print(str(input_filename) + ': Applying mask ' + str(mask_filename) +
              ' (HDU' + str(mask_hdu) + ') to Data (File ' +
              str(input_filename) +
    ...
    xax = crval3 + cdelt3 * (np.arange(length) + 1 - crpix3)
    # Angstrom - default in MUSE
    if cunit3 == 'nm':
    ...
    if not classic or ignorenoise:
        ...
        print(input_filename +
              ': Writing out spatially cross-correlated flux ... ' +
              get_timestring(starttime))
        lef.write_fitscube(filtered, cube_header, primary_header, out_filename,
                           overwrite=args.overwrite)
        print(input_filename +
              ': All Done! Written spatial cross-corelated flux to ' +
        ...
        print(input_filename + ': Reading in the Noise Cube... (HDU ' +
              str(stat_hdu) + ') ' +
              get_timestring(starttime))
        ...
        if std:
            # If the input noise cube is std (e.g., KMOS default),
        ...
        filtered_stat = spatial_smooth_lib.filter_parallel(
        ...
        print(input_filename + ': Writing out final data... ' +
              get_timestring(starttime))
        if not args.notemp:
            filtered, filtered_header = lef.read_hdu(tempfilename,
                                                     0,
                                                     memmap=memmap)
            os.remove(tempfilename)
        else:
            filtered, filtered_header = filtered, cube_header
        filtered_data_hdu = fits.ImageHDU(data=filtered,
                                          header=filtered_header)

        del filtered
        del filtered_header
        filtered_noise_hdu = fits.ImageHDU(data=filtered_stat,
                                           header=stat_head)
        out_hdu_list = fits.HDUList(
            hdus=[fits.PrimaryHDU(data=None,
    ...
    print(input_filename +
          ': All Done: lsd_cc_spatial.py output written to ' +
          out_filename + ' . ' +
          get_timestring(starttime))
