...
def create_filter_matrix(delta_lambda, lambda_start=4800,
                         lambda_em=1215.67, cdelt=1.25,
                         lMax=3463, trunc_constant=4, var_spec=None):
    ...
    # <- constant for the truncation formula, literature says
    C = trunc_constant
    ...
    tMax = abs(
        ((delta_lambda / lambda_em) *
         ((lambda_start / cdelt) + lMax)) /
        fwhm_factor)
    M = C * m.sqrt(tMax) + 1  # ...numerically sufficient (I hope..)

    # .. since the sum runs from -M to +M we have to create
    # vectors of the length 2M
    M = int(m.ceil(2 * M))
    ...
    for i in range(int(lMax) + M - 1):
        ...
        # and M << lmax ... its also not correctly written
    ...
    for i in range(int(lMax) + M - 1):
        ...
        if i < M:  # upper left corner
            ...
            filter_matrix[i, i + 1 - M:int(lMax)] =\
                h_l[i, 0:M - (i + 1 - int(lMax))]
    ...
    # but for now return the matrix as defined here

    return filter_matrix


def create_filter_matrix_vel(velocity, lambda_start=4800.,
                             cdelt=1.3, lMax=3463,
    ...
    for i in range(int(lMax) + M - 1):
        ...
        # see note in create_filter_matrix - the index i in above
        # seems not super accurate,
    ...
    for i in range(int(lMax) + M - 1):
        if i < M:
            ...
            filter_matrix[i, i + 1 - M:int(lMax)] = h_l[i,
                                                        0:M -
                                                        (i + 1 - int(lMax))]
    ...
    if var_spec is not None:
        ...
        filter_matrix = np.divide(filter_matrix,
                                  var_spec,
                                  where=np.isfinite(var_spec))
        filter_matrix = (filter_matrix.T * norm).T
...
def filter_spectrum(filter_matrix, data):
    """Dot-multiply filter matrix with each spectrum-vector
     in an array of spectrum vectors
...
def filter_parallel(filter_matrix, data, num_threads, string="spectra",
    ...
    for j in range(num_threads):
        ...
        if j + 1 == num_threads:
            ...
            endstr = str(length)
        if np.all(nans_select is not None):
