...
def calc_max(in_cube, label_cube, objects=None, windowed=False):
    ...
    Out:
    ----
    maxima ... numpy.array of maximum values
    within detection clusters of in_cube
...
def sn_ana_flux(x_i, y_i, z_i, sn_cube, flux_cube, thresh_ana,
                varcube=None, ws=20):
    """
    flux(,vari) = sn_ana_flux(x_i,y_i,z_i,
     sn_cube, flux_cube, thresh_ana, ws=20)
    ...
    varcube (optional) ... variance cube -
                if provided propagated variance will be calculated
    ws ... +/- size of the window of the subcubes where analysis is performed
               ...
               but for large objects (nearby galaxies or
                extended LyA) a bigger
               ws should be choosen)

    Out:
    ----
    flux ... sum of flux values in analysis cluster
    vari (only if variance cube is provided) ...
    propagated variance for flux
...
#     cum_sum_sort = [np.cumsum(sorted_flat) for
#     sorted_flat in sorted_flattened]
...
#                      + the number of strange things
#                      happening during the calculation
#                     (in general: the higher,
#                     the less trustworthy is the output -
...
#             if r_eff_nom < 0:  # integration might produce negative values
#             - we discard those
#                 r_eff = -999.
#                 char_rads_flag[i] += 1
#             elif r_eff_denom < 0:
...
#     # I THINK I CAN DO SOMETHING SIMILAR ... find_objects ->
...
#     Summation of flux values in elliptical apperture
#     defined via a, b, theta, r_kron.
